...
class ShuntCompensatorCreationInNodeBreakerTest extends AbstractNetworkModificationTest {
    ...
    protected void checkModification() {
        ...
        assertEquals(SHUNT_COMPENSATOR_ALREADY_EXISTS.getMessage() + " : v5shunt", exception.getMessage());
        ...
        assertEquals(CREATE_SHUNT_COMPENSATOR_ERROR.getMessage() + " : Maximum section count should be greater or equal to 1", exception.getMessage());

        // CreateWithSectionError
        modificationToCreate.setMaximumSectionCount(2);
        modificationToCreate.setSectionCount(3);
        exception = assertThrows(NetworkModificationException.class, () -> modificationToCreate.toModification().check(getNetwork()));
        assertEquals(CREATE_SHUNT_COMPENSATOR_ERROR.getMessage() + " : Section count should be between 0 and Maximum section count (2), actual : 3", exception.getMessage());
    }
